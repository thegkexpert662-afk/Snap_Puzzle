...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  Future<void> _setOnline(bool value) async {
    ...
    if (user != null) {
      await FirebaseFirestore.instance.collection('players').doc(user.uid).update({'isOnline': value});
    }
  }
  ...
  Future<void> checkBanStatus() async {
    ...
    if (doc.data()?['isBanned'] != true) return;
    showDialog<void>(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => AlertDialog(
        backgroundColor: Colors.red.shade700,
        title: const Text('🚫 ACCOUNT BANNED', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        content: const Text('Your account has been banned by the Admin.\n\nPlease contact support.\nsnappazzel.support@gmail.com', style: TextStyle(color: Colors.white)),
        actions: [
          TextButton(
            onPressed: () async {
  ...
  Future<void> loadDailyAds() async {
    ...
    final doc = await FirebaseFirestore.instance.collection('players').doc(user.uid).get();
    if (!doc.exists || !mounted) return;
    setState(() => dailyAdCount = (doc.data()?['dailyAdCount'] as num?)?.toInt() ?? 0);
  }
  ...
  Future<void> checkAndResetDailyAds() async {
    ...
    final raw = data['dailyAdResetAt'];
    if (raw == null || (raw is Timestamp && DateTime.now().isAfter(raw.toDate()))) {
      await ref.update({'dailyAdCount': 0, 'dailyAdResetAt': Timestamp.fromDate(DateTime.now().add(const Duration(hours: 24)))});
  ...
  Future<void> showRewardedAd() async {
    ...
    rewardedAd!.show(onUserEarnedReward: (ad, reward) async {
      ...
      final ref = FirebaseFirestore.instance.collection('players').doc(user.uid);
      final snap = await ref.get();
      final currentCoins = (snap.data()?['coins'] as num?)?.toInt() ?? coins;
      final currentAds = (snap.data()?['dailyAdCount'] as num?)?.toInt() ?? dailyAdCount;
      await ref.update({'coins': currentCoins + 50, 'dailyAdCount': currentAds + 1});
      if (mounted) setState(() { coins = currentCoins + 50; dailyAdCount = currentAds + 1; });
    });
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (online || state == AppLifecycleState.paused || state == AppLifecycleState.detached) _setOnline(online);
  }
  ...
  Widget _buildCoinPill() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: const LinearGradient(colors: [Color(0xFF12396D), Color(0xFF071D47)]),
        border: Border.all(color: const Color(0xFF2D79BD), width: 2),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Container(width: 34, height: 34, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFFFFF06A), Color(0xFFFFA900)])), child: const Icon(Icons.star, color: Colors.white, size: 22)),
          const SizedBox(width: 7),
          Text('$coins', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(width: 7),
          Container(width: 34, height: 34, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: LinearGradient(colors: [Color(0xFF8AF84E), Color(0xFF1BCB50)])), child: const Icon(Icons.add, color: Colors.white, size: 24)),
        ],
      ),
    );
  }
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          width: 52,
          height: 52,
          decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF284A78), Color(0xFF071A3C)]), border: Border.all(color: Colors.white24, width: 1.5)),
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: const Color(0xAA071A3C), borderRadius: BorderRadius.circular(19), border: Border.all(color: Colors.white12)),
          child: Row(children: [const Icon(Icons.campaign_rounded, color: Color(0xFF59D7FF), size: 21), const SizedBox(width: 9), Expanded(child: Marquee(text: data['text']?.toString() ?? '', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13), velocity: 35, blankSpace: 60, pauseAfterRound: const Duration(seconds: 1)))]),
        );
      },
    );
  }

  Widget _buildLogoHero() => Column(children: [Image.asset('assets/images/logo.png', width: 270, height: 125, fit: BoxFit.contain), const SizedBox(height: 4), const Text('TURN YOUR PHOTOS INTO PUZZLES', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1.3))]);
  ...
  Widget _buildBottomNavigation() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xF20A1C3B), borderRadius: BorderRadius.circular(34), border: Border.all(color: Colors.white12, width: 1.5)),
      child: Row(
          ...
          return Expanded(
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(color: selected ? const Color(0xFF0753A9) : Colors.transparent, borderRadius: BorderRadius.circular(26), border: selected ? Border.all(color: const Color(0xFF168DFF), width: 1.5) : null),
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(item.$1, color: selected ? Colors.white : Colors.white60, size: 25),
                    const SizedBox(height: 3),
                    FittedBox(child: Text(item.$2, style: TextStyle(color: selected ? Colors.white : Colors.white60, fontSize: 10, fontWeight: selected ? FontWeight.w900 : FontWeight.w600))),
                  ],
                ),
              ),
            ),
...
class _HomeBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Positioned.fill(
      child: DecoratedBox(
        decoration: const BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFF062C66), Color(0xFF0B1B3D), Color(0xFF030B1C)], stops: [0, .48, 1])),
