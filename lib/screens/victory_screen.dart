...
class VictoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final unlockText = currentLevel == 11
        ? '🔓 5×5 Puzzle Unlocked'
        : currentLevel == 41
            ? '🔓 6×6 Puzzle Unlocked'
            : currentLevel == 61
            ...
            SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(18, 18, 18, 8), decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF0D3D7E), Color(0xFF061D49)]), borderRadius: BorderRadius.circular(28), border: Border.all(color: const Color(0xFF1979D5), width: 2), boxShadow: const [BoxShadow(color: Color(0x44006DFF), blurRadius: 18, offset: Offset(0, 7))]), child: Column(children: [
                    _StatRow(Icons.timer_rounded, 'Time', '${seconds}s', const Color(0xFF4DD7FF)),
                    _StatRow(Icons.swap_horiz_rounded, 'Moves', '$moves', const Color(0xFF58E69B)),
                    _StatRow(Icons.monetization_on_rounded, 'Coins', '+$coinReward', const Color(0xFFFFD447)),
                    _StatRow(Icons.stars_rounded, 'XP', '+$xpReward', const Color(0xFFD08BFF)),
                    _StatRow(Icons.workspace_premium_rounded, 'Level', '$currentLevel', const Color(0xFFFF9D55)),
                  ])),
                  const SizedBox(height: 20),
                  _ActionButton(label: 'Next Puzzle', icon: Icons.refresh_rounded, colors: const [Color(0xFF18D890), Color(0xFF008F63)], onTap: onNextPuzzle),
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: onTap,
        icon: Icon(icon),
        label: Text(label, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
        style: ElevatedButton.styleFrom(
          backgroundColor: colors.first,
          foregroundColor: Colors.white,
          elevation: 7,
          shadowColor: colors.first.withOpacity(.45),
        ...
        child: Stack(
          children: [
            Positioned(
              top: -80,
              right: -90,
              child: _glow(240, const Color(0xFF008BFF)),
            ),
            Positioned(
              bottom: -100,
              left: -100,
              child: _glow(250, const Color(0xFF6C2CFF)),
            ),
          ],
        ),
      ),
